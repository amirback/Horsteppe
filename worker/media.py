...
def append_end_card(video: Path, card: Path, out: Path) -> None:
    ...
    """
    # Профиль финальный, а не промежуточный. Это последний видеопроход
    # сборки: карточка приклеивается уже после субтитров и музыки, то есть
    # пересжимает ВЕСЬ ролик целиком. Промежуточный профиль идёт на
    # пресете `fast`, а замер в начале файла показал, что на быстрых
    # пресетах кодек экономит на решениях и теряет больше, чем возвращает
    # низкий CRF. Оставить здесь `fast` значило бы обесценить выбор
    # `medium`, сделанный проходом раньше.
    run_ffmpeg([
        ...
        "-map", "[v]", "-map", "[a]",
        *_ENCODE_FINAL, "-c:a", "aac", "-b:a", "192k",
        str(out),
    ])
