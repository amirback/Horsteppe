...
def assign_generation_modes(shots: list[dict], mode: str = DEFAULT_MODE) -> float:
    ...
    chosen = 0.0
    # Порядок очереди за настоящим видео.
    #
    # Крючок стоит первым безусловно. Приоритет товарных кадров однажды уже
    # вытеснил его из очереди: в собранной рекламе «Вояж» первые две секунды
    # оказались зумом по фотографии, хотя бриф требует настоящее движение
    # именно в первых секундах — зритель решает, смотреть ли дальше, до того
    # как увидит товар.
    #
    # Дальше идут кадры с настоящим снимком товара: товарный кадр без
    # движения обесценивает рекламу — зритель видит фотографию там, где ждал
    # показ. Затем важность и место в фильме.
    for shot in sorted(
        shots,
        key=lambda s: (
            0 if s["purpose"] == HOOK_PURPOSE else 1,
